...
public class IndexManageTest {
    ...
    public void indexUpdate() throws Exception {
        ...
        doc.add(new TextField("title", "lucene spring springmvc redis", Field.Store.YES));

        //创建更新的条件
        Term term = new Term("id", "123");

        //使用IndexWriter对象，把符合条件的文档先删除再更新新文档
     ...
     */
    @Test
    public void indexWriteWithBoost() throws Exception {
        //采集数据
        BookDao bookDao = new BookDaoImpl();
        List<Book> bookList = bookDao.queryBookList();
        //创建文档对象（Document）
        ...
        for (Book book : bookList) {
            ...
            if ("5".equals(book.getId().toString())) {
